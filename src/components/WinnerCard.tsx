...
export const WinnerCard: FC<WinnerCardProps> = ({ winnerList }) => {
  ...
  return (
    <div>
      ...
      <div className="rounded-xl bg-indigo-800 w-full mt-9 h-0 shadow-xl relative overflow-hidden" style={ptFullStyle}>
        <div className="absolute top-0 bottom-0 left-0 right-0 text-opacity-20">
          <div className="bg-cover bg-no-repeat bg-bottom h-2/5 flex flex-col justify-center mix-blend-hard-light" style={bgStyle}>
            <i style={trophyStyle} className="bg-contain bg-no-repeat w-14 h-20 block mx-auto" />
          ...
          <div className="h-3/5 px-1 py-4 flex items-end justify-center -mx-1">
            {winnerList.map((item, index) => (
              <div className={`px-2 text-center order-${index === 0 ? '2' : index === 1 ? '1' : '3'}`} key={item.id}>
                <img src={`./images/${item.imageName!}`} alt="iu" className={`rounded-full w-4/5 mx-auto ${!index && 'transform scale-150 origin-bottom'}`} />
